...
def canny_detect(img,area_eps=0.05,ratio_threshold=0.9):
    ...
    for cnt in counters:
        ...
        for p in cnt:
            p_y=p[0][1]
            if p_y > height*1070/1080 or p_y < height*10/1080:
                drop_flag = True
                break
        if not drop_flag:
            cnts.append(cnt)
...
class pokemon_capture:
    ...
    def scan_callback(self, data):
        ...
        angle1_idx=int(math.radians(175)/angle_increment)
        angle2_idx=int(math.radians(185)/angle_increment)
        back_distance, count = 0, 0

        for dis in ranges[angle1_idx-1:angle2_idx+1]:
            if not math.isnan(dis) and not math.isinf(dis):
                back_distance += dis
                count += 1
